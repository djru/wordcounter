class ApplicationController < ActionController::Base
  ...
  helper_method :sanitize
  helper_method :analyze
  ...
  def analyze text
    ...
    text_nodes.each do |word|
      word = sanitize(word)
      if word_count.keys.include? (word + 'e')
        # we run into a situation where the gsub cuts off the trailing e of conjugated verbs ending in e (for instance, hates becomes hat, not hate), therefore, first add the possible missing e back and see if it is in the list
        word_count[word + 'e'] += 1
      elsif word_count[word]
         word_count[word] += 1
      else
        word_count[word] = 1
      end
